...
import RicaListModal from "./RicaListModal";
import Pageheader from "../shared/layouts-components/page-header/pageheader";
import { Card, Col, Row } from "react-bootstrap";
import Seo from "../shared/layouts-components/seo/seo";
import SearchBox from "@/components/SearchBox";

const OrdersRica = ({ data }) => {
  ...
  const handleEventClick = (info) => {
    ...
    const ordiniGiorno = filteredData.filter((order) => {
      if (!order["Data prevista consegna"]) return false; // Ignora ordini senza data

      const dataOrd = order["Data prevista consegna"].format("YYYY-MM-DD");
  ...
  filteredData.forEach((order) => {
    if (!order["Data prevista consegna"]) return; // Esclude ordini senza Data Cons.

    const data = order["Data prevista consegna"].format("YYYY-MM-DD");
